...
export default function MuralEmprestimos({ session }) {
    ...
    const handleBiparMatricula = async (e) => {
        ...
        if (categorias.length > 0) setCategoriaSel(categorias[0].id);

        setStep(2);
    };

    const processarEscolhaItem = async (item) => {
        if (nomeAluno) {
            await executarRetirada(item, nomeAluno);
        } else {
    ...
    const handleBiparItem = (e) => {
        ...
        if (itemEncontrado) {
            ...
            alert("Item não encontrado ou já está emprestado.");
            setInputItem('');
        }
    };

    const handleSalvarNomeENovoEmprestimo = async (e) => {
        e.preventDefault();
        if (!nomeAluno.trim()) return;
        await executarRetirada(itemSelecionado, nomeAluno.trim());
    };

    const executarRetirada = async (item, nome) => {
        const sucesso = await registrarRetirada({
        ...
        if (sucesso) resetFlow();
    };

    const handleConfirmarDevolucao = async () => {
        const sucesso = await registrarDevolucao(alunoAnalisado.emprestimoAtivo.id);
        if (sucesso) resetFlow();
    };
        ...
        <div style={{ padding: '24px', height: '100%', display: 'flex', gap: '24px', color: 'var(--text)' }}>
            <div style={{ width: '450px', background: 'var(--surface, #1e293b)', borderRadius: '8px', border: '1px solid var(--border)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                ...
                <div style={{ padding: '24px', flex: 1, display: 'flex', flexDirection: 'column', position: 'relative' }}>
                    ...
                    {step === 1 && (
                        <form onSubmit={handleBiparMatricula} style={{ display: 'flex', flexDirection: 'column', gap: '16px', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
                            <div style={{ fontSize: '48px', opacity: 0.5 }}>💳</div>
                            <p style={{ textAlign: 'center', color: 'var(--muted)', fontSize: '14px' }}>Bipe o crachá do aluno para começar</p>
                            <input
                                ...
                                style={{ width: '100%', padding: '16px', fontSize: '20px', textAlign: 'center', borderRadius: '8px', background: 'rgba(255,255,255,0.05)', color: '#fff', border: '2px dashed #3b82f6', outline: 'none' }}
                            />
                        </form>
                    )}
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', overflowY: 'auto', maxHeight: '200px', paddingRight: '4px' }}>
                                {itensDisponiveis.map(item => (
                                    <div
                                        ...
                                        style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border)', borderRadius: '8px', padding: '12px', cursor: 'pointer' }}
                                    >
                        ...
                        emprestimosAtivos.length === 0 ? (
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '12px' }}>
                                {emprestimosAtivos.map(emp => (
                                    <div key={emp.id} style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border)', borderLeft: '4px solid #f59e0b', borderRadius: '8px', padding: '16px' }}>
                                        ...
                                        <div style={{ fontSize: '13px', color: '#cbd5e1' }}>👤 {emp.nomeAluno}</div>
                                        <div style={{ fontSize: '11px', color: '#64748b', marginTop: '8px', paddingTop: '8px', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
