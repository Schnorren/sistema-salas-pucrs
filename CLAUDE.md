...
Migrations em [supabase/migrations/](supabase/migrations/) — rode **nesta ordem** no SQL Editor (dev e prod): `concluir_devolucao_rpc.sql`, `realizar_emprestimo_rpc.sql` (retirada atômica + máx. 1 empréstimo ativo por aluno), `estatisticas_emprestimos_rpc.sql`, `trocas_sala_data_aula.sql`, `trocas_sala_semana.sql`, `rls_policies.sql` (RLS multi-prédio — obrigatória), `auditoria_log.sql`. Migrations **não** rodam automaticamente; ao editar `rls_policies.sql`, reaplique (é idempotente).
